...
app.get('/ship', async (req, res) => {
  ...
  logger.info({ message: 'Ship request finished', elapsedTime, traceIds: span.getTraceIds() })

  res.send({ success: true, shipped: true, traceIds: span.getTraceIds() })
  span.log({ 'event': 'shipped' });

  span.finish()
})

expressUtil.listen(app, process.env.SHIPPING_SERVICE_PORT, process.env.SHIPPING_SERVICE_HOST)
