...
app.get('/pay', async (req, res) => {
  ...
  logger.info({ message: 'Pay request finished', elapsedTime, traceIds: span.getTraceIds() })

  res.send({ success: true, paid: true, traceIds: span.getTraceIds() })

  span.log({ 'event': 'paid' });
  span.finish()
})

expressUtil.listen(app, process.env.PAYMENT_SERVICE_PORT, process.env.PAYMENT_SERVICE_HOST)
